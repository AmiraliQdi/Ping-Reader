...
class PingViewerLogReader:
    ...
    def unpack_string(cls, file: IO[Any]):
        return cls.unpack_array(file).decode('Latin-1')
    ...
    def recover(cls, file: IO[Any]):
        ...
        while not (match := cls.TIMESTAMP_FORMAT.search(
            ...
            timestamp = roi[match.start():end].decode('Latin-1')
            amount_read -= (len(roi) - end)
            cls.failed_bytes += amount_read
        ...
        file_size = file.tell()
        cls.failed_bytes += file_size - current_pos
        raise EOFError('No timestamp match found in final recovery attempt'
                       f' - lost {cls.failed_bytes} bytes in total'
                       f' ({cls.failed_bytes / file_size:.1%} of file).')
